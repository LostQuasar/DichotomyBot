...
async def updateStatus(num):
    await botClient.change_presence(activity=discord.Activity(type=discord.ActivityType.playing, name=f'with {num}/2'))
...
async def on_ready():
    ...
    connector = buttplug.WebsocketConnector("ws://127.0.0.1:12345", logger=vibeClient.logger)
    try:
